...
$LogFile      = "$LogDir\BIOS_PasswordRemoval_$(Get-Date -Format 'yyyyMMdd').log"
...
function Remove-AllLocalState {
    ...
    Get-ChildItem "$LogDir\BIOS_Password_*.log" -ErrorAction SilentlyContinue | Remove-Item -Force
    Write-Log "INFO" "Rotation logs cleaned up"
}

# ============================================================================
# MAIN
# ============================================================================
Write-Log "INFO" "======== BIOS Password Removal (DaaS Return) ========"
...
if ($PasswordState -eq 0) {
    Write-Log "INFO" "No BIOS password currently set. Nothing to remove."
...
try {
    ...
    Write-Output "Cannot reach Entra ID - removal deferred"
    exit 1
}
...
try {
    ...
    Write-Output "Key Vault unreachable - removal deferred"
    exit 1
}
...
Write-Log "INFO" "Removing BIOS SMP..."
try {
    ...
    $r = $WMI.WmiOpcodeInterface("WmiOpcodePasswordNew01:");                 if ($r.Return -notmatch "Success") { throw "New: $($r.Return)" }
    $r = $WMI.WmiOpcodeInterface("WmiOpcodePasswordSetUpdate");              if ($r.Return -notmatch "Success") { throw "Update: $($r.Return)" }
    Write-Log "SUCCESS" "BIOS SMP removed"
} catch {
    Write-Log "ERROR" "BIOS password removal failed: $_"
    Write-Output "Removal failed"
    exit 1
}

Remove-AllLocalState
Write-Log "INFO" "All local state cleaned up"

Write-Log "SUCCESS" "Password removal complete for $Serial"
Write-Output "BIOS password removed - device ready for return"
exit 0
